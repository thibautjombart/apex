...
## CLASS DESCRIPTION:
## - instance of multidna store list of DNA alignments
...
setClass("multidna", representation(dna="listOrNULL", meta="data.frameOrNULL"),
         prototype(dna=NULL, meta=NULL))

setClassUnion("multidnaOrNULL", c("multidna", "NULL"))
...
##
setMethod("initialize", "multidna", function(.Object, dna=NULL, individualID=NULL,
    ...
    if(is.null(labels) || length(labels)!=NSEQ){
        if(!quiet) cat("\n[multidna constructor] missing/incomplete labels provided - using generic labels.\n")
    ...
    if(is.null(individualID) || is.null(date)){
        ...
        if(NFIELDS>=3){
            if(!quiet) cat("\n[multidna constructor] extracting individualID and dates from labels.\n")

            temp <- strsplit(labels, split=sep, fixed=TRUE)
            if(!all(sapply(temp, length) == NFIELDS)) {
                warning("[multidna constructor] Improper labels (varying numbers of fields)")
                cat("\nCulprits are:\n")
    ...
    if(is.null(individualID)){
        if(!quiet) cat("\n[multidna constructor] note: missing individualID\n")
    ...
    ## HANDLE DATE ##
    if(is.null(date)){
        if(!quiet) cat("\n[multidna constructor] note: missing dates\n")
        date <- rep(NA, NSEQ)
    }
...
## new("multidna") # empty object
## new("multidna", woodmouse) # no locus info
## new("multidna", as.matrix(woodmouse), locus=rep(c('loc1', 'loc2', 'locXX'), c(10,4,1)))


## ## test accessors
## x <- new("multidna", as.matrix(woodmouse), locus=rep(c('loc1', 'loc2', 'locXX'), c(10,4,1)))
## get.dna(x, locus=1)
